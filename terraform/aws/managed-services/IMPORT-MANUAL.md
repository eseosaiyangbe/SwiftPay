...
- `aws_db_parameter_group.swiftpay`
...
```bash
terraform import aws_db_instance.swiftpay swiftpay-postgres
terraform import aws_elasticache_replication_group.swiftpay swiftpay-redis
```

MQ broker uses **broker ID** (e.g. `b-xxxx`), not the name. Get it, then import:

```bash
aws mq list-brokers --region us-east-1 --query 'BrokerSummaries[?BrokerName==`swiftpay-rabbitmq`].BrokerId' --output text
terraform import aws_mq_broker.swiftpay <broker-id>
```
...
| Resource | Import ID |
|----------|-----------|
| `aws_db_subnet_group.swiftpay` | `swiftpay-db-subnet-group` |
| `aws_security_group.rds` | `sg-07f760b2016d3eea1` (or lookup by name in your VPC) |
| `aws_db_parameter_group.swiftpay` | `swiftpay-postgres-params-dev` |
| `aws_elasticache_subnet_group.swiftpay` | `swiftpay-redis-subnet-group` |
| `aws_security_group.elasticache` | `sg-0844446ee14310cd6` |
| `aws_cloudwatch_log_group.redis` | `/aws/elasticache/redis/swiftpay` |
| `aws_security_group.mq` | `sg-032f12c523564367e` |
| `aws_db_instance.swiftpay` | `swiftpay-postgres` |
| `aws_elasticache_replication_group.swiftpay` | `swiftpay-redis` |
| `aws_mq_broker.swiftpay` | `<broker-id>` from `aws mq list-brokers` |
