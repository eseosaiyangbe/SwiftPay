...
#       az aks get-credentials --resource-group swiftpay-rg --name swiftpay-aks
...
if [[ -z "${ACR_NAME:-}" ]]; then
  ...
  ACR_NAME=$(az acr list --query "[?contains(name, 'swiftpay')].name" -o tsv 2>/dev/null | head -1)
  if [[ -z "$ACR_NAME" ]]; then
...
echo "Deploying SwiftPay to AKS"
echo "  ACR       : $ACR_NAME"
echo "  Tag       : $IMAGE_TAG"
echo ""
